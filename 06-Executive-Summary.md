# Executive Summary — Board and Leadership

**Document:** 06-Executive-Summary.md | **Assessment:** OmniCare Health IT Governance Gap Assessment | **Prepared for:** Board of Directors, Executive Leadership Team | **Date:** Q2 2026

---

## Assessment Overview

This report presents the findings of a comprehensive IT governance gap assessment conducted at OmniCare Health during Q2 2026. The assessment evaluated **19 COBIT 2019 governance and management objectives** across five domains, mapped each finding to **COSO Internal Control — Integrated Framework (2013) principles**, and developed a prioritized remediation roadmap.

### Why This Assessment Was Undertaken

OmniCare Health has grown from a focused telemedicine provider to a $200M healthcare technology enterprise through three acquisitions in 24 months. This rapid scaling has introduced:

- Fragmented IT decision-making across business units
- Redundant technology investments (3 EHR platforms, 2 identity systems)
- No IT steering committee or governance oversight
- Undocumented IT general controls — a material weakness risk for SOX readiness
- Ad-hoc risk management exposed by a 2025 ransomware incident

**The board and executive leadership commissioned this assessment to baseline current governance maturity and build a roadmap to achieve SOX readiness and enterprise risk management maturity.**

---

## Overall Governance Maturity

| Metric | Result |
|---|---|
| **Current Average Maturity** | **1.37 / 5.0** (Performed / Ad-hoc) |
| **Target Average Maturity** | **3.16 / 5.0** (Defined / Standardized) |
| **Average Maturity Gap** | **1.79** |
| **Objectives Below Target** | **19 of 19** (100%) |
| **Critical-Risk Findings** | **4** |
| **High-Risk Findings** | **7** |
| **Medium-Risk Findings** | **4** |

### Maturity by Domain

| Domain | Current | Target | Gap | Assessment |
|---|---|---|---|---|
| **Governance (EDM)** | **1.20** | **3.00** | **1.80** | Foundational governance structures absent |
| **Align, Plan, Organise (APO)** | **1.43** | **3.29** | **1.86** | Strategy and risk management are ad-hoc |
| **Build, Acquire, Implement (BAI)** | **1.50** | **3.00** | **1.50** | Project management and integration weak |
| **Deliver, Service, Support (DSS)** | **2.00** | **3.50** | **1.50** | Operations basic but functional |
| **Monitor, Evaluate, Assess (MEA)** | **1.00** | **3.00** | **2.00** | Controls and compliance not established |

---

## Key Strengths

While the overall maturity is low, the assessment identified several areas OmniCare can build upon:

1. **Executive awareness and commitment** — Leadership recognises the governance gap and has sponsored this assessment. The CEO, CFO, and CTO are aligned on the need for change.
2. **Technical engineering capability** — The engineering team follows sound development practices (CI/CD, code review). BAI03 scored at Level 2, the highest in the BAI domain.
3. **Basic security foundation** — A CISO and security team exist with fundamental controls deployed (antivirus, firewall, MFA). This provides a base to build a comprehensive security program.
4. **Financial discipline** — IT budget management, while siloed, is practiced at the departmental level. This indicates capability to adopt enterprise-level resource management.
5. **Regulatory awareness** — The compliance officer has identified key obligations. The function needs resourcing but the awareness exists.

---

## Critical Gaps

### Four Critical-Risk Findings Requiring Immediate Attention

| # | Finding | Domain | Current | Target | Gap | COSO Principle |
|---|---|---|---|---|---|---|
| **F01** | No Formal IT Governance Framework | EDM01, APO01 | **1** | **3** | **2** | P1, P2, P3 |
| **F02** | Absence of IT Steering Committee | EDM01, EDM05 | **1** | **3** | **2** | P1, P2, P16 |
| **F05** | No Enterprise IT Risk Management | EDM03, APO12 | **1** | **3** | **2** | P6, P7 |
| **F08** | Inadequate IT Security Governance | APO13, DSS05 | **2** | **4** | **2** | P7, P11, P12 |

### Additional High-Risk Findings

| # | Finding | Risk Rating |
|---|---|---|
| **F03** | No IT Strategy Aligned to Business | **High** |
| **F04** | Fragmented IT Decision-Making | **High** |
| **F06** | No Formal Benefits Realisation Process | **High** |
| **F07** | Post-Merger IT Integration Challenges | **High** |
| **F11** | No IT Performance Monitoring | **High** |
| **F12** | Lack of Internal Control Framework (SOX) | **High** |
| **F13** | Compliance Management Gaps | **High** |

---

## Top 5 Recommendations

| Rank | Recommendation | Cost | Timeline | Maturity Uplift |
|---|---|---|---|---|
| **1** | **Establish IT Governance Framework & Steering Committee** (R01) | $150K–$200K | 3 months | EDM01: 1→3, EDM05: 1→3 |
| **2** | **Implement Enterprise IT Risk Management** (R03) | $120K–$180K | 4 months | EDM03: 1→3, APO12: 1→3 |
| **3** | **Launch IT Security Governance Program** (R05) | $250K–$400K | 6 months | APO13: 2→4, DSS05: 2→4 |
| **4** | **Build Internal Control Framework for SOX Readiness** (R09) | $300K–$500K | 8 months | MEA02: 1→3 |
| **5** | **Develop IT Strategy Aligned to Business Objectives** (R02) | $100K–$150K | 4 months | APO02: 2→4 |

### Business Impact of Top 5 Recommendations

| Recommendation | Business Impact |
|---|---|
| R01 — Governance Framework | Ends fragmented IT decisions; enables controlled technology spend; provides governance infrastructure for all other initiatives |
| R03 — Risk Management | Reduces regulatory exposure; enables informed risk-taking; lowers cyber insurance premiums (estimated 15–20% reduction) |
| R05 — Security Governance | Reduces data breach risk for 400K+ patient records; supports HIPAA compliance; protects brand reputation |
| R09 — SOX Internal Controls | Addresses material weakness risk; enables SOX Section 404 certification; avoids adverse auditor opinions |
| R02 — IT Strategy | Aligns $10M+ annual IT spend to business priorities; enables data-driven investment decisions |

---

## Target State Vision (18 Months)

```
Current State (Q2 2026)                   Target State (Q4 2027)
┌─────────────────────┐                   ┌─────────────────────────┐
│  Avg Maturity: 1.47 │                   │  Avg Maturity: 3.16     │
│  Ad-hoc governance  │                   │  Defined governance     │
│  No ITSC            │  ──────────────►  │  IT Steering Committee  │
│  Fragmented IT      │   12–18 months    │  Centralised decisions  │
│  No risk process    │   12 initiatives  │  Embedded risk mgmt     │
│  No internal ctrl   │   $1.6M–$2.5M     │  SOX-ready controls     │
│  Weak security gov  │                   │  Measured security      │
└─────────────────────┘                   └─────────────────────────┘
```

### What Success Looks Like

| Dimension | Current | Target |
|---|---|---|
| **Governance** | No governance framework | Board-approved IT governance charter with active ITSC |
| **Strategy** | Annual operating plan only | 3-year IT strategic plan linked to enterprise goals |
| **Risk** | Informal risk discussions | Quarterly risk assessments with documented risk register |
| **Security** | Basic tools, no ISMS | ISO 27001-aligned security program with metrics |
| **Controls** | No ITGC documentation | Tested IT general controls for SOX compliance |
| **Monitoring** | No performance data | Executive dashboard with 12+ KPIs reported monthly |
| **Compliance** | Manual, 1 FTE | Automated compliance monitoring with sufficient resourcing |
| **Cost Efficiency** | 5.2% IT spend/revenue | 3.8–4.2% IT spend/revenue (industry benchmark) |

---

## Investment Summary

| Phase | Timeframe | Investment | Maturity Uplift |
|---|---|---|---|
| **Phase 1 — Quick Wins** | Months 1–3 | $450K–$650K | +0.4 maturity |
| **Phase 2 — Medium-Term** | Months 4–8 | $650K–$1.1M | +0.8 maturity |
| **Phase 3 — Strategic** | Months 9–18 | $600K–$800K | +0.5 maturity |
| **Total** | **18 months** | **$1.6M–$2.5M** | **1.47 → 3.16** |

**Estimated annual benefit:** $4.1M–$6.8M (cost savings, risk reduction, operational efficiency)

**Expected payback period:** 6–12 months from initial investment

---

## Recommended Next Steps

| Step | Action | Owner | Timeline |
|---|---|---|---|
| 1 | Board review and approval of this assessment | CEO | 2 weeks |
| 2 | Appoint IT Governance Sponsor (recommend CFO or CTO) | CEO | 2 weeks |
| 3 | Approve Phase 1 budget ($450K–$650K) | Board | 1 month |
| 4 | Launch R01 — Governance Framework and ITSC formation | Sponsor | Immediate |
| 5 | Launch R03 — Risk Management Process implementation | Sponsor | Month 1 |
| 6 | Launch R05 — Security Governance Program | CISO | Month 1 |
| 7 | Initiate R09 — SOX Internal Controls workstream | CFO | Month 2 |
| 8 | Schedule 6-month maturity reassessment | Internal Audit | Month 6 |

---

## Conclusion

OmniCare Health's IT governance maturity of **1.47/5.0** is characteristic of a rapidly scaled organisation that has outgrown its informal governance model. The gap to the target of **3.16** is significant but achievable within **12–18 months** with executive commitment and a structured investment of **$1.6M–$2.5M**.

The four critical-risk findings — absence of governance framework, no steering committee, no risk management process, and inadequate security governance — must be addressed as a priority. These foundational gaps underpin all other deficiencies and are prerequisites for SOX readiness.

Critically, the recommended investment of **$1.6M–$2.5M** is expected to deliver **$4.1M–$6.8M** in annual benefits through cost optimisation (reducing IT spend from 5.2% to 3.8% of revenue), risk reduction (lower insurance premiums, avoided incidents), and SOX compliance (avoiding material weakness findings).

**The cost of inaction** — continued regulatory exposure, fragmented IT costs, inability to achieve SOX compliance, and elevated security risk — far exceeds the investment required to reach a defined, standardized governance state.

---

## Document Control

| Attribute | Detail |
|---|---|
| **Document** | OmniCare Health — IT Governance Gap Assessment Executive Summary |
| **Version** | 1.0 |
| **Date** | Q2 2026 |
| **Framework** | COBIT 2019 / COSO Internal Control — Integrated Framework (2013) |
| **Prepared By** | Internal Audit |
| **Distribution** | Board of Directors, Executive Leadership Team |
| **Next Review** | Q4 2026 (6-month maturity reassessment) |

---

**End of Executive Summary.** For detailed findings, see companion documents:

- [02-Maturity-Ratings.md](./02-Maturity-Ratings.md) — Detailed maturity assessment for all 19 objectives
- [03-Gap-Analysis.md](./03-Gap-Analysis.md) — Complete gap descriptions and risk ratings
- [04-Prioritized-Recommendations.md](./04-Prioritized-Recommendations.md) — Implementation roadmap
- [05-COSO-Mapping.md](./05-COSO-Mapping.md) — COSO 2013 principle mapping
